...
* 三相モーター式の場合はリレーを使うようにスケッチや装置を変更するのがよい．
 ...
 * 設定：
   ...
   * RESET: 側窓を一旦全閉し，その後自動制御に移行する．
     * RTCを実装している場合は，側窓のズレを解消するために日付が変更になるタイミングで自動的にRESETを実行することも可能（実装しているが無効化している）．
...
## 停電時の動作
V1.0.0では停電後復旧すると一旦リセット動作後に元の動作状態に移行した．手動停止時だと，停電復旧後に全閉状態となった．全開状態で停止していた場合などは都合が悪いので，V1.1.0では動作を以下の考え方に変更した．

1. 停電復旧時は基本的に元の動作モードで復旧する．
1. 手動開、手動閉、手動停止時は基本的にそれに従って動作する（停電復旧後も）．
   1. いかなる場合でも手動開，手動閉，手動停止はそれに従わないとトラブル時には不適切との考え
1. 巻上カウンタは手動停止時以外は停電復旧後リセットされる．
   1. 動作モード設定時にしかバックアップしていないので復旧できない．
1. 自動制御は必ず電源ON後に一度リセット動作している必要がある．
   1. 巻上カウンタがずれていると全開，全閉にならない場合があるため．

このため，実際の動作は以下の通りとなる．

1. 自動制御時に停電復旧すると一旦リセット後に自動制御に移行する．
1. 手動停止時に停電復旧すると元の手動停止状態に復旧する．巻上カウンタも復旧する．
1. 上記以外の動作モード時に停電復旧すると元の動作モードに復旧するが，巻上カウンタはリセット（手動閉時は最大値）される．
1. 手動開、手動閉時で停電復旧後に自動制御に設定すると一旦リセット後に自動制御に移行する．
