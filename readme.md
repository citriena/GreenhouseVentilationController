...
* 設定した最低温度に応じて加熱機を起動／停止する．
 * 応差（ヒステリシス）の設定も可能
* 測定した気温をmicroSDカードに記録可能
 * 加えて加熱機の起動／停止時刻の記録も行う．
...
### 自作専用シールド
* サーミスタ接続端子，加熱機制御用出力端子，降雨センサ接続端子，microSDカードモジュール，RTCモジュール実装
 ...
 * 設定範囲：0-30（℃）
 * 初期値：5
 * 説明：この設定温度以下になると加熱機を動作させる．基本的にはON/OFF制御である．応差（ヒステリシス）動作可能．デフォルトでは応差動作無．応差感度の設定はスケッチ内で行うため，設定変更には再コンパイルが必要．なお，Arduinoの加温出力制限入力ピン（デフォルト3番ピン）をLOW（GNDと接続）にするとPWMにより加温出力を調整できる（ただし，PWMに対応した加熱機のみ）．私の都合により実装した機能で，必要とする方はほとんどいないでしょうが．
 ...
 * 設定範囲：0-600（秒）
 * 初期値：150
 * 説明：累積巻上時間が上限に達したら巻上を停止する．実際の巻上上限は巻上モーターのリミッタで設定し、この累積巻上上限はそれよりも少し多めに設定するのが良い。使っているモーターによっては、時間だけで制御していると停止位置がずれてくる．
 ...
 * 設定範囲：0～100（%）
 * 初期値：70
 * 説明：Arduinoの加温出力制限入力ピン（デフォルト3番ピン）をLOW（GNDと接続）にするとPWMにより加温出力を調整する．このため，電熱線等のPWM制御可能な加熱機以外では使用不可．また，実際の出力制御にはSSR（半導体リレー）などPWMに対応した装置で加熱機を制御することが必要である．当然ながら機械式リレーでは使用できない．

* #define HYSTERESIS
 * 内容：加熱動作時の応差温度（ヒステリシス）
 * 設定範囲：0～5程度（特に制限は無い）
 * 初期値：0
 * 説明：加熱動作時の応差温度を設定する．加温設定温度以下になると加温開始するが，加温停止は加温設定温度＋応差温度以上になったときである．
